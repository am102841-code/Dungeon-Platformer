...
class Enemy():
    def __init__(self, platform_x, platform_width, platform_height):
        self.x = platform_x 
        self.y = platform_height 
        self.move_speed = 4
        self.on_ground = False
        self.health = 5
        self.enemy_rect = pygame.Rect(self.x, self.y, 65, 65)
        self.facing_left = True
        self.enemy_image = pygame.image.load('ENEMY.png') 
        self.enemy_image = pygame.transform.scale(self.enemy_image, (65, 65))
        self.enemy_flipped_image = pygame.transform.flip(self.enemy_image, True, False)
        self.enemy_now = self.enemy_image
        self.health = 2 
        self.enemy_reset = False 
        self.on_ground = False 
        self.hitbox = self.enemy_image.get_rect() 
        self.move_speed = 4/2 # half of player speed 
        self.platform_x = platform_x 
        self.platform_width = platform_width 
        self.platform_height = platform_height 
        self.left_bound = platform_x 
        self.right_bound = platform_x + platform_width - self.enemy_rect.width 
        self.move_timer = 0 
    ...
    def ai(self):
        ...
        # Set image based on facing direction
        if self.facing_left:
            self.enemy_now = self.enemy_image
        else:
            self.enemy_now = self.enemy_flipped_image
        ...
        # Creator Page 
          ...
          if exit_button.hitbox.collidepoint((mouse_x, mouse_y)) and mouseClicked:
            ...
            if level == 2 and level_changing:
                ...
                enemy = Enemy(platform_x=ob3.left, platform_width=ob3.width, platform_height=ob3.top - 65)
                enemy.x = ob3.left + 10 
                ...
                pygame.draw.rect(WINDOW, (255, 0, 0), enemy.enemy_rect)
            if level == 2 and not level_changing:
                if player.hitbox.colliderect(ob5):
                    ...
                    if enemy and enemy not None:
                        enemy.ai()
                        WINDOW.blit(enemy.enemy_now, (enemy.x, enemy.y))
            ...
            if level == 3 and level_changing == True:
                ...
                ob2 = pygame.Rect(450 + 75 - 15, 290, 50, 50) 
                #ob3 = pygame.Rect(330 + 75, 175, 50, 50)
